...
const update_title_bar_progress = (stage = "") => {
  ...
  switch( stage ){
    ...
    case 'distribute':
      title = `ENU | Snapshot ${debug.rates.percent_complete}% | ${debug.registry.accepted+debug.registry.rejected}/${debug.registry.total}`
    break
    case 'registry':
      title = `ENU | Building Registry | ${registrants.length}`
    break
    case 'reclaimer':
      title = `ENU | Reclaimable TXs | ${ debug.reclaimable.total }`
    break
    case 'complete':
      title = `ENU | Snapshot Complete`
    break
    default:
      title = `ENU | Welcome`
    break
...
ui_refresh = setInterval( () => {
  ...
  if( disconnected ) {
    ...
    document.getElementsByClassName('disconnected')[0].innerHTML = `
      ...
      See <a href="https://github.com/enumivo/enumivo/blob/master/programs/snapshot/readme.md">Readme</a> for more information
      </p>
